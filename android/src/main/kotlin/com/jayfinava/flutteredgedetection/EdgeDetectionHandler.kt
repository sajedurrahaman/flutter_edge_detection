...
class EdgeDetectionHandler : MethodCallHandler, PluginRegistry.ActivityResultListener {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?): Boolean {
        if (requestCode == REQUEST_CODE) {
            when (resultCode) {
                Activity.RESULT_OK -> {
                    // New multi-page result format from ScanActivity:
                    // - "uris": ArrayList<String> of saved image file paths
                    // - "pdf_uri": String content Uri
                    val uris = data?.getStringArrayListExtra("uris")
                    val pdfPath = data?.getStringExtra("pdf_path")
                    val mode = data?.getStringExtra("mode")
                    val openCrop = data?.getBooleanExtra("open_crop", false) ?: false
                    if ((uris != null && uris.isNotEmpty()) || pdfPath != null || mode != null) {
                        val payload: HashMap<String, Any?> = hashMapOf(
                            "uris" to uris,
                            "pdf_path" to pdfPath,
                            "mode" to mode,
                            "open_crop" to openCrop,
                        )
    ...
    private fun openCameraActivity(call: MethodCall, result: Result) {
        ...
        val bundle = Bundle().apply {
            ...
            putBoolean(AUTO_CAPTURE, call.argument<Boolean>(AUTO_CAPTURE) ?: true)
            putString(INITIAL_MODE, call.argument<String>("initial_mode") ?: "documents")
        }
